...
export function redirectToGoogleOAuth() {
  ...
  const clientId = process.env.NEXT_PUBLIC_CLIENT_ID!;
  const redirectUrl = `${process.env.NEXT_PUBLIC_VERCEL_DOMAIN}/auth/callback/google`;  
...
export const redirectToMicrosoftOAuth = () => {
  const clientId = process.env.NEXT_PUBLIC_CLIENT_ID!;
  const domain = "https://nibraspool-app.auth.us-east-1.amazoncognito.com";
  const redirectUrl = `${process.env.NEXT_PUBLIC_VERCEL_DOMAIN}/auth/callback/microsoft`; 
